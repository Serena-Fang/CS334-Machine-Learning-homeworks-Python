...
class RandomForest(object):
    ...
    def train(self, xFeat, y):
        ...
        for i in range(self.nest):
            ...
            yHat = np.argmax(y_preds, axis=1)
            # print(yOut.shape, y_preds.shape)
            # for y_p in y_probs:
...
def main():
    ...
    # Question (a)
    # Build the adaptation of the random forest
    np.random.seed(args.seed)   
    model = RandomForest(nest=args.nest, maxFeat=args.maxFeat, criterion=args.criterion,
    ...
    for maxFeat in maxFeats:
        rf = RandomForest(nest=best_nest, maxFeat=maxFeat, criterion=args.criterion,
                          maxDepth=args.maxDepth, minLeafSample=args.minLeafSample)
        errors = cross_val_error(xTrain, yTrain, rf)
    ...
    for i in range(len(maxDepths)):
        ...
        for j in range(len(minLeafSamples)):
            ...
            rf = RandomForest(nest=best_nest, maxFeat=best_maxFeat, criterion=args.criterion,
                              maxDepth=maxDepth, minLeafSample=minLeafSample)
    ...
    # Question (c)
    # train the random forest model using the optimal parameters
    print("training the model using the optimal parameters")
    bestRF = RandomForest(nest=best_nest, maxFeat=best_maxFeat, criterion=args.criterion,
                      maxDepth=best_maxDepth, minLeafSample=best_minLeafSample)
